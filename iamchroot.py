...
#run("pacman -Syu neovim bat fd exa ripgrep neofetch", shell=True)

# Configure pacman
print("Configure pacman as needed.", end="")
input()
...
# Localization
print("Uncomment any locales need.", end="")
input()
...
# Install boot loader
#run("pacman -S efibootmgr refind amd-ucode intel-ucode", shell=True)
...
# Add default user
#run("pacman -S zsh openrc-zsh-completions zsh-autosuggestions zsh-completions zsh-syntax-highlighting", shell=True)
...
print("Uncomment the line to allow users of group wheel to execute any command.")
input()
#run("EDITOR=nvim visudo", shell=True)

# Other stuff you should do or you'll be sad
#run("pacman -S dhcpcd wpa_supplicant connman-openrc", shell=True)
#run("rc-update add connmand", shell=True)
#run("nvim /etc", shell=True)
print("MOTD", end=": ")
motd = input().strip()
#run(f"printf '\n{motd}\n' > /etc/motd", shell=True)
...
print("  HOOKS=(... autodetect keyboard keymap modconf block encrypt openswap resume filesystems ...)")
input()
#run("nvim /etc/mkinitcpio.conf", shell=True)
#run("mkinitcpio -P", shell=True)

# Unmount and complete
#run("umount -R /mnt", shell=True)

print("\nTasks completed. You should reboot.")
